...
String disassemble(CodeDump code) {
  ...
  writeLine(".version 0x${(code.versionMajor * 16 + code.versionMinor).toRadixString(16)}");
  ...
  void writeFunc(Prototype func, String name) {
    ...
    if (func.lineStart != 0 || func.lineEnd != 0) writeLine(".lines " + func.lineStart.toString() + " " + func.lineEnd.toString());
    ...
    void writeConst(int i, [String comment]) {
      writeLine(".const " + i.toString() + " " + func.constantScope.elementAt(i - 1).toString() + (comment == null ? "" : " ; $comment"));
    }
    ...
    for (int i = 0; i < func.upvals.length; i++) {
      writeLine(".upval " + (func.upvals[i].stack ? "0" : "1") + " " + func.upvals[i].reg.toString() + (func.upvals[i].name == null ? "" : " \"" + luaEscape(func.upvals[i].name) + "\""));
    }
    ...
    if (local_start.containsKey(0)) {
      local_start[0].forEach((e) => writeLine(".local " + (clocal++).toString() + " \"" + luaEscape(e.name) + "\""));
    }
    
    for (int i = 0; i < func.code.length; i++) {
      var c = func.code[i];
      if (local_end.containsKey(i + 1)) {
        local_end[i + 1].forEach((e) => writeLine(".endlocal " + (--clocal).toString()));
      }
      
      if (local_start.containsKey(i + 1)) {
        local_start[i + 1].forEach((e) => writeLine(".local " + (clocal++).toString() + " \"" + luaEscape(e.name) + "\""));
      }
